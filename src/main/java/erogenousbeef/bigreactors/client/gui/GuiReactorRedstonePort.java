...
public class GuiReactorRedstonePort extends BeefGuiBase {
    ...
    public void initGui() {
        ...
        for (CircuitType ct : CircuitType.values()) {
            if (ct == CircuitType.DISABLED) {
                continue;
            }
            GuiSelectableButton newBtn = new GuiSelectableButton(
                buttonOrdinal++,
                leftX,
                topY,
                reactorPartBlock.getRedNetConfigIcon(ct),
                0xFF00FF00,
                this);
        ...
        if (currentCircuitType == CircuitType.inputSetControlRod) {
            subInputRodSetting
                .setText(Integer.toString(TileEntityReactorRedstonePort.unpackControlRodLevelOn(this.outputLevel)));
            subInputRodSettingOff
                .setText(Integer.toString(TileEntityReactorRedstonePort.unpackControlRodLevelOff(this.outputLevel)));
    ...
    public void updateScreen() {
        ...
        if (selectedSetting == port.getCircuitType()) {
            ...
            if (this.activeOnPulse != port.isInputActiveOnPulse() || this.greaterThan != port.getGreaterThan()
    ...
    protected void actionPerformed(GuiButton clickedButton) {
        if (clickedButton.id == 0) {
            ...
            CommonPacketHandler.INSTANCE.sendToServer(
                new ReactorRedstonePortChangeMessage(
                    port,
                    newCircuitType.ordinal(),
                    actualOutputLevel,
                    this.greaterThan,
                    this.activeOnPulse));
            ...
            for (Entry<CircuitType, GuiSelectableButton> pair : btnMap.entrySet()) {
                pair.getValue()
                    .setSelected(pair.getKey() == port.getCircuitType());
            }

            setSubSettingsToDefaults(port.getCircuitType());
        } else if (clickedButton.id == 2) {
            CircuitType selectedCircuitType = this.getUserSelectedCircuitType();
            if (TileEntityReactorRedNetPort.isInput(selectedCircuitType)) this.activeOnPulse = !this.activeOnPulse;
            else this.greaterThan = !this.greaterThan;
            ...
            && clickedButton.id < MINIMUM_SETTING_SELECTOR_ID + btnMap.size()) {
    ...
    protected void keyTyped(char inputChar, int keyCode) {
        boolean isAnyTextboxFocused = this.subInputRodSetting.isFocused() || this.subInputRodSettingOff.isFocused()
