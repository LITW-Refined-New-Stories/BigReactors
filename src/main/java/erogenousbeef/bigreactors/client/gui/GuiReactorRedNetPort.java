...
public class GuiReactorRedNetPort extends BeefGuiBase {
    ...
    protected static final String[] channelLabelStrings = new String[] { "White", "Orange", "Magenta", "LightBlue",
        "Yellow", "Lime", "Pink", "Gray", "LightGray", "Cyan", "Purple", "Blue", "Brown", "Green", "Red", "Black" };

    public static final String[] grabbableTooltips = { "Input: Toggle reactor on/off",
        "Input: Change control rod insertion", "Input: Eject Waste", "Output: Fuel Temp (C)", "Output: Casing Temp (C)",
        "Output: Fuel mix (% fuel, 0-100)", "Output: Fuel amount", "Output: Waste amount",
    ...
    public void initGui() {
        ...
        for (int i = 0; i < channelLabelStrings.length; i += 2) {
            channelSelectors[i] = new BeefGuiRedNetChannelSelector(
                this,
                channelLabelStrings[i],
                i,
                leftX,
                topY,
                60,
            ...
            channelSelectors[i
                + 1] = new BeefGuiRedNetChannelSelector(this, channelLabelStrings[i + 1], i + 1, leftX, topY, 60, 20);
        ...
        for (int i = 1; i < circuitTypes.length; i++) {
            grabbables[i - 1] = new RedNetConfigGrabbable(
                grabbableTooltips[i - 1],
                reactorPartBlock.getRedNetConfigIcon(circuitTypes[i]),
                circuitTypes[i]);
    ...
    protected boolean hasSettingChanged(int idx) {
        return grabTargets[idx].hasChanged() || hasSubSettingChanged(idx)
            || (port.isInputActivatedOnPulse(idx) != pulseActivated[idx]);
    }
    ...
    public void onChannelChanged(int changedChannel) {
        ...
        } else if (TileEntityReactorRedNetPort.isInput(currentCircuitType)
            && CircuitType.canBeToggledBetweenPulseAndNormal(currentCircuitType)) {
    ...
    private String getControlRodLabelFromLocation(CircuitType circuitType, CoordTriplet location) {
        if (location == null) {
            ...
            TileEntity te = port.getWorldObj()
    ...
    private void changeSubSetting(boolean forward) {
        ...
        if (CircuitType.hasCoordinate(circuitType)) {
            ...
            CoordTriplet[] controlRodLocations = port.getReactorController()
                .getControlRodLocations();
            int newIdx = 0;
            // Locate current idx; will be -1 if not found, which is expected.
            int oldIdx = Arrays.asList(controlRodLocations)
                .indexOf(subSettingCoords[selectedChannel]);
            if (forward) {
                newIdx = oldIdx + 1;
            } else {
                if (oldIdx == -1) {
    ...
    private void updateSubSettingValueText() {
        ...
        if (CircuitType.hasCoordinate(circuitType)) {
            subSettingValueString
                .setLabelText(getControlRodLabelFromLocation(circuitType, subSettingCoords[selectedChannel]));
        } else if (TileEntityReactorRedNetPort.isInput(circuitType)
            && CircuitType.canBeToggledBetweenPulseAndNormal(circuitType)) {
