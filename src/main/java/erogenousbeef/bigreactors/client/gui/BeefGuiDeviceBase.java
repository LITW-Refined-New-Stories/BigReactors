...
public abstract class BeefGuiDeviceBase extends BeefGuiBase {
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (button.id >= EXPOSURE_BUTTON_ID_BASE && button.id < EXPOSURE_BUTTON_ID_BASE + 6) {
            ...
            CommonPacketHandler.INSTANCE.sendToServer(
                new DeviceChangeExposureMessage(
                    _entity.xCoord,
                    _entity.yCoord,
                    _entity.zCoord,
                    button.id - EXPOSURE_BUTTON_ID_BASE,
                    true));
        }
    }
    ...
    protected void updateInventoryExposures() {
        ...
        for (int side = 0; side < 6; side++) {
            ...
            exposureButtons[side]
                .setIcon(deviceBlock.getIconFromTileEntity(_entity, BlockBRDevice.META_CYANITE_REPROCESSOR, side));
        }
    }
}
