...
public class RotorSimpleRenderer implements ISimpleBlockRenderingHandler {
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
        ...
        if (te instanceof TileEntityTurbineRotorPart) {
            ...
            if (rotorPart.isConnected() && rotorPart.getTurbine()
                .isAssembled()
                && rotorPart.getTurbine()
                    .getActive()) {
    ...
    public static void renderRotorShaft(Block block, RenderBlocks renderer, int metadata, ForgeDirection majorAxis,
        boolean[] hasBlades, int x, int y, int z, boolean drawOuterRectangle) {
    ...
    private void renderBladeFromWorld(RenderBlocks renderer, IBlockAccess world, int x, int y, int z, Block block,
        int metadata) {
        TileEntity te = world.getTileEntity(x, y, z);
        ...
        if (rotorDir == ForgeDirection.UNKNOWN) {
            ...
            while (rotorDir == ForgeDirection.UNKNOWN && !bladeDirs.isEmpty()) {
                ForgeDirection dir = bladeDirs.remove(bladeDirs.size() - 1); // Trim off the end to avoid shifting crap
                                                                             // in memory
                ...
                int dist = 0;
                while (world.getBlock(curX, curY, curZ) == block && dist < 32) { // only go up to 32 blocks in any
                                                                                 // direction without finding a rotor,
                                                                                 // for sanity
    ...
    public static void renderBlade(RenderBlocks renderer, int x, int y, int z, Block block, int metadata,
        ForgeDirection rotorDir) {
        if (rotorDir == ForgeDirection.UNKNOWN) {
    ...
    private static ForgeDirection findRotorMajorAxis(IBlockAccess world, int x, int y, int z, Block block) {
        ...
        for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            if (world.getBlock(x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ) == block && BlockTurbineRotorPart
                .isRotorShaft(world.getBlockMetadata(x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ))) {
                retDir = dir;
                break;
            }
        }
    ...
    private static boolean[] findBlades(IBlockAccess world, int x, int y, int z, Block block,
        ForgeDirection majorAxis) {
        ...
        for (int i = 0; i < dirsToCheck.length; i++) {
            ...
            if (world.getBlock(x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ) == block && BlockTurbineRotorPart
                .isRotorBlade(world.getBlockMetadata(x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ))) {
                ret[i] = true;
            } else {
                ret[i] = false;
            }
    ...
    private static ForgeDirection[][] normals = {
        { ForgeDirection.UNKNOWN, ForgeDirection.UNKNOWN, ForgeDirection.WEST, ForgeDirection.EAST,
            ForgeDirection.NORTH, ForgeDirection.SOUTH }, // DOWN
        { ForgeDirection.UNKNOWN, ForgeDirection.UNKNOWN, ForgeDirection.WEST, ForgeDirection.EAST,
            ForgeDirection.NORTH, ForgeDirection.SOUTH }, // UP
        { ForgeDirection.EAST, ForgeDirection.WEST, ForgeDirection.UNKNOWN, ForgeDirection.UNKNOWN, ForgeDirection.DOWN,
            ForgeDirection.UP }, // NORTH
        { ForgeDirection.EAST, ForgeDirection.WEST, ForgeDirection.UNKNOWN, ForgeDirection.UNKNOWN, ForgeDirection.DOWN,
            ForgeDirection.UP }, // SOUTH
        { ForgeDirection.SOUTH, ForgeDirection.NORTH, ForgeDirection.DOWN, ForgeDirection.UP, ForgeDirection.UNKNOWN,
            ForgeDirection.UNKNOWN }, // WEST
        { ForgeDirection.SOUTH, ForgeDirection.NORTH, ForgeDirection.DOWN, ForgeDirection.UP, ForgeDirection.UNKNOWN,
            ForgeDirection.UNKNOWN }, // EAST
    };
    ...
    private static void renderRotorBladeConnection(RenderBlocks renderer, Block block, int metadata,
        ...
        rotorOffsets[0] = rotorOffsets[3] = rotorOffsets[4] = rotorOffsets[7] = 0.5D
            + (rotorDirMultiplier * rotorDirectionOffset);
        rotorOffsets[1] = rotorOffsets[2] = rotorOffsets[5] = rotorOffsets[6] = 0.5D
            - (rotorDirMultiplier * rotorDirectionOffset);

        double bladeOffsets[] = new double[8];
        bladeOffsets[0] = bladeOffsets[1] = bladeOffsets[2] = bladeOffsets[3] = 0.5D
            + (bladeDirMultiplier * bladeInnerOffset);
        bladeOffsets[4] = bladeOffsets[5] = bladeOffsets[6] = bladeOffsets[7] = 0.5D
            + (bladeDirMultiplier * bladeOuterOffset);
    ...
    private static final int[][] quadSet1 = { { 4, 5, 6, 7 }, // Outer rectangular face of the rotor
        { 7, 3, 0, 4 }, // "top" rhombus
    ...
    private static final int[][] quadSet2 = { { 7, 6, 5, 4 }, // Outer rectangular face of the rotor
        { 4, 0, 3, 7 }, // "top" rhombus
