...
public class RotorSpecialRenderer extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileEntity tileentity, double x, double y, double z, float f) {
        ...
        if (!turbine.isAssembled() || !turbine.getActive() || !turbine.hasGlass()) {
            return;
        }

        Integer displayList = bearing.getDisplayList();
        ForgeDirection rotorDir = bearing.getOutwardsDir()
            .getOpposite();
    ...
    int generateRotor(RotorInfo rotorInfo) {
        ...
        while (rotorIdx < rotorInfo.rotorLength) {
            ...
            RotorSimpleRenderer.renderRotorShaft(
                BigReactors.blockTurbineRotorPart,
                renderBlocks,
                BlockTurbineRotorPart.METADATA_SHAFT,
                rotorDir,
                hasBlades,
                currentRotorCoord.x,
                currentRotorCoord.y,
            ...
            for (int bladeIdx = 0; bladeIdx < bladeDirs.length; bladeIdx++) {
                ...
                while (bladeLen < rotorInfo.bladeLengths[rotorIdx][bladeIdx]) {
                    RotorSimpleRenderer.renderBlade(
                        renderBlocks,
                        bladeCoord.x,
                        bladeCoord.y,
                        bladeCoord.z,
                        BigReactors.blockTurbineRotorPart,
                        BlockTurbineRotorPart.METADATA_BLADE,
                        rotorInfo.rotorDirection);
                    bladeLen++;
                    bladeCoord.translate(bladeDirs[bladeIdx]);
                }
            }
