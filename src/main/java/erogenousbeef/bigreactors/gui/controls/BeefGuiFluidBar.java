...
public class BeefGuiFluidBar extends BeefGuiIconProgressBar implements IBeefTooltipControl {
    ...
    protected IIcon getProgressBarIcon() {
        ...
        if (tanks != null && tankIdx < tanks.length) {
            if (tanks[tankIdx].fluid != null) {
                return tanks[tankIdx].fluid.getFluid()
                    .getIcon();
            }
        }
        return null;
    ...
    public String[] getTooltip() {
        ...
        if (tanks != null && tankIdx < tanks.length) {
            FluidStack tankFluid = tanks[tankIdx].fluid;
            if (tankFluid != null) {
                String fluidName = tankFluid.getFluid()
                    .getLocalizedName(tankFluid);
                if (tankFluid.getFluid()
                    .getID() == FluidRegistry.WATER.getID()) {
                    fluidName = "Water";
                } else if (tankFluid.getFluid()
                    .getID() == FluidRegistry.LAVA.getID()) {
                        fluidName = "Lava";
                    }

                return new String[] { fluidName,
                    String.format("%d / %d mB", tankFluid.amount, tanks[tankIdx].capacity) };
            } else {
