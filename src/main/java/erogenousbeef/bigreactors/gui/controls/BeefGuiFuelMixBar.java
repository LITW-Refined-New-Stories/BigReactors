...
public class BeefGuiFuelMixBar extends BeefGuiVerticalProgressBar implements IBeefTooltipControl {
    ...
    protected void drawProgressBar(Tessellator tessellator, TextureManager renderEngine, int barMinX, int barMaxX,
        ...
        double fuelProportion = (double) entity.getFuelAmount()
            / (double) (entity.getFuelAmount() + entity.getWasteAmount());
        double wasteProportion = (double) entity.getWasteAmount()
            / (double) (entity.getFuelAmount() + entity.getWasteAmount());
        ...
        if (fuelProportion > 0) {
            ...
            tessellator.addVertexWithUV(
                this.absoluteX + this.width,
                this.absoluteY + this.height - 1,
                zLevel,
                fuelRightU,
                fuelMaxV);
            tessellator.addVertexWithUV(
                this.absoluteX + this.width,
                this.absoluteY + this.height - barHeight,
                zLevel,
                fuelRightU,
                fuelMinV);
            tessellator
                .addVertexWithUV(this.absoluteX, this.absoluteY + this.height - barHeight, zLevel, fuelLeftU, fuelMinV);
            tessellator.draw();
        }
    ...
    public String[] getTooltip() {
        ...
        if (entity.getFuelAmount() + entity.getWasteAmount() == 0) {
            ...
            depletion = ((float) entity.getWasteAmount() / (float) (entity.getFuelAmount() + entity.getWasteAmount()))
                * 100f;
        ...
        return new String[] { EnumChatFormatting.AQUA + "Core Fuel Status", String.format(" %2.1f%% full", fullness),
            String.format(" %2.1f%% depleted", depletion), "", String.format("Fuel Rods: %d", entity.getFuelRodCount()),
            String.format("Max Capacity: %d mB", entity.getCapacity()), String.format("Fuel: %s", fuelString),
