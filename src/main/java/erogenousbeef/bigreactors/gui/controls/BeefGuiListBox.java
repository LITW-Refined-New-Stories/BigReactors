...
public class BeefGuiListBox extends BeefGuiControlBase {
    ...
    public void drawBackground(TextureManager renderEngine, int mouseX, int mouseY) {
        ...
        drawRect(
            absoluteX + margin,
            absoluteY + margin,
            absoluteX + width - margin * 2,
            absoluteY + height - margin * 2,
            backgroundColor);
    }
    ...
    public void drawForeground(TextureManager renderEngine, int mouseX, int mouseY) {
        ...
        for (int i = displayTop; i < entries.size(); i++) {
            entry = entries.get(i);
            if (entry.getHeight() + drawnY > this.height) {
                break;
            } else {
                if (this.selectedEntryIdx == i) {
                    entry.draw(
                        this.guiContainer.getFontRenderer(),
                        relativeX + margin,
                        relativeY + margin + drawnY,
                        selectedLineColor,
                        selectedTextColor);
                } else {
                    entry.draw(
    ...
    public void onMouseClicked(int mouseX, int mouseY, int mouseButton) {
        int checkedY = 0;
        for (int i = displayTop; i < entries.size(); i++) {
            int entryHeight = entries.get(i)
                .getHeight();
            if (checkedY + entryHeight > (this.height - margin * 2)) {
                break;
            }

            if (this.absoluteY + margin + checkedY <= mouseY
                && this.absoluteY + margin + entryHeight + checkedY >= mouseY) {
                setSelectedIndex(i);
