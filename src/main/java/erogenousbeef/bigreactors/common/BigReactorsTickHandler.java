...
public class BigReactorsTickHandler {
    ...
    public void addRegenChunk(int dimensionId, ChunkCoordIntPair chunkCoord) {
        ...
        if (!chunkRegenMap.containsKey(dimensionId)) {
            ...
            if (!chunkRegenMap.get(dimensionId)
                .contains(chunkCoord)) {
                chunkRegenMap.get(dimensionId)
                    .add(chunkCoord);
            }
        }
    }
        ...
        if (event.side == Side.SERVER && event.phase == TickEvent.Phase.END) {
            ...
            if (chunkRegenMap.containsKey(dimensionId)) {
                ...
                while (System.nanoTime() - startTime < maximumDeltaTimeNanoSecs && !chunksToGen.isEmpty()) {
                    ...
                    fmlRandom
                        .setSeed((xSeed * nextChunk.chunkXPos + zSeed * nextChunk.chunkZPos) ^ event.world.getSeed());

                    BigReactors.worldGenerator
                        .generateChunk(fmlRandom, nextChunk.chunkXPos, nextChunk.chunkZPos, event.world);
                }
