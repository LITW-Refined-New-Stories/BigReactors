...
public class BlockTurbinePart extends BlockContainer implements IPeripheralProvider {
    ...
    private static final String[] _subBlocks = new String[] { "housing", "controller", "powerTap", "fluidPort",
        "bearing", "computerPort" };
    ...
    private static final String[] _subIconNames = new String[] { "housing.edge.0", "housing.edge.1", "housing.edge.2",
        "housing.edge.3", "housing.face", "housing.corner", "controller.idle", "controller.active",
        "powerTap.connected", "fluidPort.outlet" };
    ...
    public void registerBlockIcons(IIconRegister par1IconRegister) {
        ...
        for (int i = 0; i < _subBlocks.length; ++i) {
            _icons[i] = par1IconRegister
                .registerIcon(BigReactors.TEXTURE_NAME_PREFIX + getUnlocalizedName() + "." + _subBlocks[i]);
        }

        for (int i = 0; i < _subIcons.length; i++) {
            _subIcons[i] = par1IconRegister
                .registerIcon(BigReactors.TEXTURE_NAME_PREFIX + getUnlocalizedName() + "." + _subIconNames[i]);
        }
        ...
        if (te instanceof TileEntityTurbinePartBase) {
            ...
            if (metadata == METADATA_FLUIDPORT) {
                if (te instanceof TileEntityTurbineFluidPort) {
                    if (turbine == null || !turbine.isAssembled()
                        || part.getOutwardsDir()
                            .ordinal() == side) {
                        if (((TileEntityTurbineFluidPort) te).getFlowDirection() == FluidFlow.Out)
                            return _subIcons[SUBICON_FLUIDPORT_OUTPUT];
                        else return _icons[METADATA_FLUIDPORT];
                    } else if (turbine.isAssembled() && part.getOutwardsDir()
                        .ordinal() != side) return _subIcons[SUBICON_HOUSING_FACE];
                }
                    ...
                    .ordinal() == side) {
                        ...
                        if (metadata == METADATA_CONTROLLER) {
                            ...
                            if (te instanceof TileEntityTurbinePowerTap
                                && ((TileEntityTurbinePowerTap) te).isAttachedToPowerNetwork()) {
    ...
    private int getSubIconForHousing(IBlockAccess blockAccess, int x, int y, int z, MultiblockTurbine turbine,
        int side) {
        ...
        if (extremes >= 3) {
            ...
            for (int i = 0; i < dirsToCheck.length; i++) {
                ...
                if (neighborBlock != myBlock && neighborBlock != BigReactors.blockMultiblockGlass
                    && (BigReactors.blockMultiblockCreativePart != null
                        && neighborBlock != BigReactors.blockMultiblockCreativePart)) {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7,
                ...
                if (controller == null) {
                    player.addChatMessage(
                        new ChatComponentText(
                            String.format("SERIOUS ERROR - server part @ %d, %d, %d has no controller!", x, y, z))); // TODO
                                                                                                                     // Localize
                } else {
                    ...
                    if (e != null) {
                        player.addChatMessage(
                            new ChatComponentText(
                                e.getMessage() + " - controller " + Integer.toString(controller.hashCode()))); // Would
                                                                                                               // it be
                                                                                                               // worth
                                                                                                               // it to
                                                                                                               // localize
        ...
        if (!part.isConnected() || !part.getMultiblockController()
            .isAssembled()) {
            return false;
        }
                ...
                do {
                    ...
                    if (itemstack.getTagCompound() != null) {
                        entityitem.getEntityItem()
                            .setTagCompound(itemstack.getTagCompound());
                    }
        ...
        if (metadata == METADATA_BEARING) {
            ...
            if (te instanceof TileEntityTurbinePartStandard) {
                ...
                if (turbine != null && turbine.getActive()) {
                    ...
                    ForgeDirection inwardsDir = bearing.getOutwardsDir()
                        .getOpposite();
                    CoordTriplet minCoord, maxCoord;
                    minCoord = turbine.getMinimumCoord();
                    maxCoord = turbine.getMaximumCoord();
                    ...
                    for (int i = 0; i < numParticles; i++) {
                        ...
                        world.spawnParticle(
                            BigReactors.isValentinesDay ? "heart" : "cloud",
                            particleX,
                            particleY,
                            particleZ,
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        if (metadata == METADATA_BEARING) {
            ...
            if (te instanceof TileEntityTurbineRotorBearing) {
                ...
                if (bearing.isConnected() && bearing.getTurbine()
                    .getActive()) {
    ...
    public IPeripheral getPeripheral(World world, int x, int y, int z, int side) {
        ...
        if (te instanceof TileEntityTurbineComputerPort) return (IPeripheral) te;
