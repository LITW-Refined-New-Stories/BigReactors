...
public class BlockReactorPart extends BlockContainer implements IRedNetOmniNode, IPeripheralProvider {
    ...
    private static String[] _subBlocks = new String[] { "casing", "controller", "controlRod", "powerTap", "accessPort",
        "coolantPort", "redNetPort", "computerPort" };
    ...
    public static boolean isPowerTap(int metadata) {
        return metadata == METADATA_POWERTAP;
    }
    ...
    public void registerBlockIcons(IIconRegister par1IconRegister) {
        ...
        for (int metadata = 0; metadata < _states.length; ++metadata) {
            ...
            for (int state = 0; state < blockStates.length; state++) {
                _icons[metadata][state] = par1IconRegister
                    .registerIcon(prefix + _subBlocks[metadata] + "." + blockStates[state]);
            }
        }
    ...
    public IPeripheral getPeripheral(World world, int x, int y, int z, int side) {
        ...
        if (te instanceof TileEntityReactorComputerPort) return (IPeripheral) te;
