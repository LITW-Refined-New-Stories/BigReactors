...
public abstract class FluidHelper implements IConditionalUpdater {
    ...
    private static final int minimumDevianceForUpdate = 50; // at least 50mB difference before we send a fueling update
                                                            // to the client
    ...
    /**
     * @param separate True if capacity is applied to each fluid separately, false if they should be treated like a
     *                 single tank with multiple fluids inside.
     */
    ...
    protected NBTTagCompound writeToNBT(NBTTagCompound destination) {
        ...
        if (tankNames.length != fluids.length) {
            throw new IllegalArgumentException(
                "getNBTTankNames must return the same number of strings as there are fluid stacks");
        }
    ...
    public FluidStack drain(int idx, FluidStack resource, boolean doDrain) {
        if (resource == null || resource.amount <= 0 || idx < 0 || idx >= fluids.length) {
            return null;
        }

        Fluid existingFluid = getFluidType(idx);
        if (existingFluid == null || existingFluid.getID() != resource.getFluid()
            .getID()) {
            return null;
        }
    ...
    public String getDebugInfo() {
        ...
        sb.append("Capacity (per): ")
            .append(Integer.toString(getCapacity()));
        String[] tankNames = getNBTTankNames();
        for (int i = 0; i < fluids.length; i++) {
            sb.append("[")
            ...
            if (fluids[i] == null) {
                ...
                sb.append(
                    stack.getFluid()
                        .getName())
                    .append(", ")
                    .append(Integer.toString(stack.amount))
                    .append(" mB");
            }
            sb.append("\n");
        }
