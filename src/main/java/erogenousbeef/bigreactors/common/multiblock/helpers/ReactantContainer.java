...
public abstract class ReactantContainer implements IConditionalUpdater {
    ...
    private static final float minimumDevianceForUpdate = 0.05f; // at least 5% difference before we send a fueling
    ...
    protected boolean canAddToStack(int idx, String incoming) {
        if (idx < 0 || idx >= tanks.length || incoming == null) {
            return false;
        ...
        for (int i = 0; i < tankNames.length; i++) {
            ...
            if (hasReactant) {
                ...
                if (!Reactants.isKnown(reactantName)) {
                    BRLog.warning(
                        "Read an unknown reactant <%s> from a network message; tank %s will remain empty",
                        reactantName,
                        tankNames[i]);
                } else {
