...
public class TileEntityReactorControlRod extends TileEntityReactorPart {
    ...
    public void setControlRodInsertion(short newInsertion) {
        if (newInsertion > maxInsertion || newInsertion < minInsertion || newInsertion == controlRodInsertion) {
            return;
        }
        if (!isConnected()) {
            return;
    ...
    protected void sendControlRodUpdate() {
        ...
        CommonPacketHandler.INSTANCE.sendToAllAround(
            new ControlRodUpdateMessage(xCoord, yCoord, zCoord, controlRodInsertion),
    ...
    public void isGoodForFrame() throws MultiblockValidationException {
        throw new MultiblockValidationException(
            String.format("%d, %d, %d - Control rods may only be placed on the top face", xCoord, yCoord, zCoord));
    }
    ...
    public void isGoodForTop() throws MultiblockValidationException {
        ...
        if (!(teBelow instanceof TileEntityReactorFuelRod)) {
            throw new MultiblockValidationException(
                String.format(
                    "%d, %d, %d - Control rods may only be placed on the top face, atop a column of fuel rods",
                    xCoord,
                    yCoord,
                    zCoord));
        }
