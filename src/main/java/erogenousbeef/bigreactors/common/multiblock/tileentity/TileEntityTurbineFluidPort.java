...
public class TileEntityTurbineFluidPort extends TileEntityTurbinePartStandard
    implements IFluidHandler, INeighborUpdatableEntity, ITickableMultiblockPart {
    ...
    public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain) {
        if (resource == null || !isConnected() || from != getOutwardsDir()) {
            return resource;
        }
    ...
    private int getTankIndex() {
        if (flowSetting == FluidFlow.In) {
            return MultiblockTurbine.TANK_INPUT;
        } else {
            return MultiblockTurbine.TANK_OUTPUT;
        }
    }
    ...
    public void onMultiblockServerTick() {
        ...
        if (pumpDestination == null || flowSetting != FluidFlow.Out) return;
    ...
    protected void checkForAdjacentTank() {
        ...
        TileEntity neighbor = worldObj
            .getTileEntity(xCoord + outDir.offsetX, yCoord + outDir.offsetY, zCoord + outDir.offsetZ);
