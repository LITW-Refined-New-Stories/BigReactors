...
public class TileEntityReactorRedNetPort extends TileEntityReactorPart
    implements ITickableMultiblockPart, INeighborUpdatableEntity {
    ...
    public int getValueForChannel(int channel) {
        ...
        switch (channelCircuitTypes[channel]) {
            ...
            case outputFuelMix:
                ...
                return (int) Math
                    .floor(((float) controller.getFuelAmount() / (float) controller.getCapacity()) * 100.0f);
    ...
    @Override
    public void onNeighborTileChange(IBlockAccess world, int x, int y, int z, int neighborX, int neighborY,
        int neighborZ) {
        checkForConnections(world, x, y, z);
    }
    ...
    protected NBTTagCompound encodeSetting(int channel) {
        ...
        if (isInput(this.channelCircuitTypes[channel])
            && CircuitType.canBeToggledBetweenPulseAndNormal(this.channelCircuitTypes[channel])) {
        ...
        for (int i = 0; i < changes.length; i++) {
            ...
            if (CircuitType.hasCoordinate(newType)) {
                ...
                if (coord != null) {
                    ...
                    if (!(te instanceof TileEntityReactorControlRod)) {
                        BRLog.warning(
                            "Invalid tile entity reference at coordinate %s - rednet circuit expected a control rod",
                            coord);
                        coord = null;
                    }
                }
