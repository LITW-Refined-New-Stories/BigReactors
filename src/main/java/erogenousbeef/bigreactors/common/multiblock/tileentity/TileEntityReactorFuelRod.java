...
public class TileEntityReactorFuelRod extends TileEntityReactorPartBase implements IRadiationModerator, IHeatEntity {
    ...
    public void moderateRadiation(RadiationData data, RadiationPacket radiation) {
        ...
        float controlRodInsertion = Math
            .min(1f, Math.max(0f, ((float) ((TileEntityReactorControlRod) te).getControlRodInsertion()) / 100f));

        // Fuel absorptiveness is determined by control rod + a heat modifier.
        // Starts at 1 and decays towards 0.05, reaching 0.6 at 1000 and just under 0.2 at 2000. Inflection point at
        // about 500-600.
        // Harder radiation makes absorption more difficult.
        float baseAbsorption = (float) (1.0 - (0.95 * Math.exp(-10 * Math.exp(-0.0022 * heat))))
            * (1f - (radiation.hardness / getFuelHardnessDivisor()));
        ...
        fuelModerationFactor += fuelModerationFactor * controlRodInsertion + controlRodInsertion; // Full insertion
                                                                                                  // doubles the
                                                                                                  // moderation factor
                                                                                                  // of the fuel as well
                                                                                                  // as adding its own
    ...
    // 1, upwards. How well does this fuel moderate, but not stop, radiation? Anything under 1.5 is "poor", 2-2.5 is
    // "good", above 4 is "excellent".
    ...
    public void isGoodForFrame() throws MultiblockValidationException {
        throw new MultiblockValidationException(
            String.format("%d, %d, %d - fuel rods may only be placed in the reactor interior", xCoord, yCoord, zCoord));
    }
    ...
    public void isGoodForInterior() throws MultiblockValidationException {
        ...
        if (!(entityAbove instanceof TileEntityReactorFuelRod || entityAbove instanceof TileEntityReactorControlRod)) {
            throw new MultiblockValidationException(
                String.format(
                    "Fuel rod at %d, %d, %d must be part of a vertical column that reaches the entire height of the reactor, with a control rod on top.",
                    xCoord,
                    yCoord,
                    zCoord));
        }
    ...
    /**
     * Returns the rate of heat transfer from this block to the reactor environment, based on this block's surrounding
     * blocks.
    ...
    public float getHeatTransferRate() {
        ...
        for (ForgeDirection dir : StaticUtils.CardinalDirections) {
            ...
            if (te instanceof TileEntityReactorFuelRod) {
                ...
                int metadata = worldObj
                    .getBlockMetadata(xCoord + dir.offsetX, yCoord + dir.offsetY, zCoord + dir.offsetZ);
    ...
    private float getConductivityFromBlock(Block block, int metadata) {
        ...
        if (block == Blocks.iron_block) {
            ...
            interiorData = ReactorInterior
                .getBlockData(ItemHelper.oreProxy.getOreName(new ItemStack(block, 1, metadata)));
