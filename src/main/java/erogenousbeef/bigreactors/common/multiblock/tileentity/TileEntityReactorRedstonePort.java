...
public class TileEntityReactorRedstonePort extends TileEntityReactorPartBase implements ITickableMultiblockPart {
    ...
    public void sendRedstoneUpdate() {
        if (this.worldObj != null && !this.worldObj.isRemote) {
            ...
            if (this.isOutput()) {
                md = isRedstoneActive() ? BlockReactorRedstonePort.META_REDSTONE_LIT
                    : BlockReactorRedstonePort.META_REDSTONE_UNLIT;
            } else {
                md = isExternallyPowered ? BlockReactorRedstonePort.META_REDSTONE_LIT
                    : BlockReactorRedstonePort.META_REDSTONE_UNLIT;
            }
    ...
    public void onNeighborBlockChange(int x, int y, int z, Block neighborBlock) {
        ...
        if (this.isInput()) {
            ...
            boolean nowPowered = isReceivingRedstonePowerFrom(
                worldObj,
                xCoord + out.offsetX,
                yCoord + out.offsetY,
                zCoord + out.offsetZ,
                ...
                if (this.isInputActiveOnPulse()) {
                    if (this.isExternallyPowered) {
                        if (this.shouldSetControlRodsInsteadOfChange()) {
                            ...
                            reactor.changeAllControlRodInsertionValues((short) this.outputLevel); // Can be negative,
                                                                                                  // don't want to mask.
                        }
                    }
                } else {
    ...
    public int getControlRodLevelWhileOff() {
        return ((outputLevel & 0xFF00) >> 8) & 0xFF;
    }

    public int getControlRodLevelWhileOn() {
        return outputLevel & 0xFF;
    }
     ...
     * @param greaterThan For outputs, whether to activate when greater than or less than the outputLevel value. For
     *                    input/control rods, whether to set (true) or change (false) the values.
     */
    ...
    private boolean isReceivingRedstonePowerFrom(World world, int x, int y, int z, ForgeDirection dir,
        ...
        if (neighborBlock == Blocks.redstone_wire) {
            ...
            return world.getIndirectPowerOutput(x, y, z, dir.ordinal())
                || world.isBlockProvidingPowerTo(x, y, z, dir.ordinal()) > 0;
        }
    }
    ...
    public void onMultiblockServerTick() {
        ...
        if (ticksSinceLastUpdate < BigReactors.ticksPerRedstoneUpdate) {
            return;
        }
    ...
    public void isGoodForFrame() throws MultiblockValidationException {
        throw new MultiblockValidationException(
            String.format(
                "%d, %d, %d - Redstone ports may only be placed on a reactor's external side faces, not as part of the frame",
                xCoord,
                yCoord,
                zCoord));
    }
    ...
    public void isGoodForTop() throws MultiblockValidationException {
        throw new MultiblockValidationException(
            String.format(
                "%d, %d, %d - Redstone ports may only be placed on a reactor's external side faces, not the top",
                xCoord,
                yCoord,
                zCoord));
    }
    ...
    public void isGoodForBottom() throws MultiblockValidationException {
        throw new MultiblockValidationException(
            String.format(
                "%d, %d, %d - Redstone ports may only be placed on a reactor's external side faces, not the bottom",
                xCoord,
                yCoord,
                zCoord));
    }
    ...
    public void isGoodForInterior() throws MultiblockValidationException {
        throw new MultiblockValidationException(
            String.format(
                "%d, %d, %d - Redstone ports may not be placed in a reactor's interior",
                xCoord,
                yCoord,
                zCoord));
    }
