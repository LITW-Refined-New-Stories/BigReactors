...
public class MultiblockTurbine extends RectangularMultiblockControllerBase
    implements IEnergyProvider, IMultipleFluidHandler, ISlotlessUpdater, IActivateable {
    ...
    float inductionEfficiency = 0.5f; // Final energy rectification efficiency. Averaged based on coil material and
                                      // shape. 0.25-0.5 = iron, 0.75-0.9 = diamond, 1 = perfect.
    float inductionEnergyExponentBonus = 1f; // Exponential bonus to energy generation. Use this for very rare materials
                                             // or special constructs.
    ...
    public static float inductorBaseDragCoefficient = 0.1f; // RF/t extracted per coil block, multiplied by rotor speed
                                                            // squared.
    public static final float baseBladeDragCoefficient = 0.00025f; // RF/t base lost to aero drag per blade block.
                                                                   // Includes a 50% reduction to factor in constant
                                                                   // parts of the drag equation
    ...
    private static final ForgeDirection[] RotorXBladeDirections = new ForgeDirection[] { ForgeDirection.UP,
        ForgeDirection.SOUTH, ForgeDirection.DOWN, ForgeDirection.NORTH };
    private static final ForgeDirection[] RotorZBladeDirections = new ForgeDirection[] { ForgeDirection.UP,
        ForgeDirection.EAST, ForgeDirection.DOWN, ForgeDirection.WEST };
    ...
    public MultiblockTurbine(World world) {
        ...
        for (int i = 0; i < NUM_TANKS; i++) tanks[i] = new FluidTank(TANK_SIZE);
        ...
        rpmUpdateTracker = new FloatUpdateTracker(100, 5, 10f, 100f); // Minimum 10RPM difference for slow updates, if
                                                                      // change > 100 RPM, update every 5 ticks
    }
    ...
    protected void isMachineWhole() throws MultiblockValidationException {
        ...
        TileEntityTurbinePartBase rotorPart = attachedRotorBearings.iterator()
            .next();
        ...
        ForgeDirection rotorDir = rotorPart.getOutwardsDir()
            .getOpposite();
        ...
        // Figure out where the rotor ends and which directions are normal to the rotor's 4 faces (this is where blades
        // emit from)
        ...
        while (!rotorShafts.isEmpty() && !rotorCoord.equals(endRotorCoord)) {
            ...
            if (!rotorShafts.remove(rotorCoord)) {
                throw new MultiblockValidationException(
                    String.format(
                        "%s - This block must contain a rotor. The rotor must begin at the bearing and run the entire length of the turbine",
                        rotorCoord));
            }
            ...
            for (ForgeDirection bladeDir : bladeDirections) {
                ...
                while (rotorBlades.remove(checkCoord)) {
                    ...
                    if (encounteredCoils) {
                        throw new MultiblockValidationException(
                            String.format(
                                "%s - Rotor blades must be placed closer to the rotor bearing than all other parts inside a turbine",
                                checkCoord));
                    }
                ...
                if (!foundABlade) {
                    if (foundCoils.remove(checkCoord)) {
                        ...
                        if (encounteredBlades) {
                            throw new MultiblockValidationException(
                                String.format(
                                    "%s - Metal blocks must by placed further from the rotor bearing than all rotor blades",
                                    checkCoord));
                        }

                        // Check the two coil spots in the 'corners', which are permitted if they're connected to the
                        // main rotor coil somehow
        ...
        if (!rotorCoord.equals(endRotorCoord)) {
            throw new MultiblockValidationException(
                "The rotor shaft must extend the entire length of the turbine interior.");
        }

        // Ensure that we encountered all the rotor, blade and coil blocks. If not, there's loose stuff inside the
        // turbine.
        if (!rotorShafts.isEmpty()) {
            throw new MultiblockValidationException(
                String.format(
                    "Found %d rotor blocks that are not attached to the main rotor. All rotor blocks must be in a column extending the entire length of the turbine, starting from the bearing.",
                    rotorShafts.size()));
        }
        ...
        if (!rotorBlades.isEmpty()) {
            throw new MultiblockValidationException(
                String.format(
                    "Found %d rotor blades that are not attached to the rotor. All rotor blades must extend continuously from the rotor's shaft.",
                    rotorBlades.size()));
        }
        ...
        if (!foundCoils.isEmpty()) {
            throw new MultiblockValidationException(
                String.format(
                    "Found %d metal blocks which were not in a ring around the rotor. All metal blocks must be in rings, or partial rings, around the rotor.",
                    foundCoils.size()));
        }
    ...
    protected void isBlockGoodForInterior(World world, int x, int y, int z) throws MultiblockValidationException {
        ...
        throw new MultiblockValidationException(
            String.format(
                "%d, %d, %d is invalid for a turbine interior. Only rotor parts, metal blocks and empty space are allowed.",
                x,
                y,
                z));
    }
    ...
    protected void onAssimilate(MultiblockControllerBase otherMachine) {
        if (!(otherMachine instanceof MultiblockTurbine)) {
            BRLog.warning(
                "[%s] Turbine @ %s is attempting to assimilate a non-Turbine machine! That machine's data will be lost!",
    ...
    protected boolean updateServer() {
        ...
        if (steamIn > 0 || rotorEnergy > 0) {
            ...
            float energyToGenerate = (float) Math.pow(inductionTorque, inductionEnergyExponentBonus)
                * inductionEfficiency;
    ...
    public void serialize(ByteBuf buf) {
        ...
        int inputFluidID, inputFluidAmt, outputFluidID, outputFluidAmt;
        {
            ...
            if (inputFluid == null || inputFluid.amount <= 0) {
                ...
                inputFluidID = inputFluid.getFluid()
                    .getID();
            ...
            if (outputFluid == null || outputFluid.amount <= 0) {
                ...
                outputFluidID = outputFluid.getFluid()
                    .getID();
    ...
    public void deserialize(ByteBuf buf) {
        ...
        if (inputFluidID == FLUID_NONE || inputFluidAmt <= 0) {
            ...
            if (fluid == null) {
                BRLog.warning(
                    "[CLIENT] Multiblock Turbine received an unknown fluid of type %d, setting input tank to empty",
                    inputFluidID);
                tanks[TANK_INPUT].setFluid(null);
            } else {
        ...
        if (outputFluidID == FLUID_NONE || outputFluidAmt <= 0) {
            ...
            if (fluid == null) {
                BRLog.warning(
                    "[CLIENT] Multiblock Turbine received an unknown fluid of type %d, setting output tank to empty",
                    outputFluidID);
                tanks[TANK_OUTPUT].setFluid(null);
            } else {
    ...
    private CoilPartData getCoilPartData(int x, int y, int z, Block block, int metadata) {
        ...
        if (block == Blocks.iron_block) {
            return TurbineCoil.getBlockData("blockIron");
        }
        if (block == Blocks.gold_block) {
            return TurbineCoil.getBlockData("blockGold");
        }

        if (block == BigReactors.blockMetal && metadata == BlockBRMetal.METADATA_LUDICRITE) {
            return TurbineCoil.getBlockData("blockLudicrite");
        }
    ...
    public void setVentStatus(VentStatus newStatus, boolean markReferenceCoordDirty) {
        ...
        if (markReferenceCoordDirty) markReferenceCoordDirty();
    }
    ...
    public ForgeDirection getRotorDirection() {
        ...
        TileEntityTurbineRotorBearing rotorBearing = attachedRotorBearings.iterator()
            .next();
        return rotorBearing.getOutwardsDir()
            .getOpposite();
    }
    ...
    public String getDebugInfo() {
        ...
        sb.append("Assembled: ")
            .append(Boolean.toString(isAssembled()))
            .append("\n");
        sb.append("Attached Blocks: ")
            .append(Integer.toString(connectedParts.size()))
            .append("\n");
        if (getLastValidationException() != null) {
            sb.append("Validation Exception:\n")
                .append(getLastValidationException().getMessage())
                .append("\n");
        }

        if (isAssembled()) {
            sb.append("\nActive: ")
                .append(Boolean.toString(getActive()));
            sb.append("\nStored Energy: ")
                .append(Float.toString(getEnergyStored()));
            sb.append("\nRotor Energy: ")
                .append(Float.toString(rotorEnergy));
            sb.append("\nRotor Speed: ")
                .append(Float.toString(getRotorSpeed()))
                .append(" rpm");
            sb.append("\nInductor Engaged: ")
                .append(Boolean.toString(inductorEngaged));
            sb.append("\nVent Status: ")
                .append(ventStatus.toString());
            sb.append("\nMax Intake Rate: ")
                .append(Integer.toString(maxIntakeRate));
            sb.append("\nCoil Size: ")
                .append(Integer.toString(coilSize));
            sb.append("\nRotor Mass: ")
                .append(Integer.toString(rotorMass));
            sb.append("\nBlade SurfArea: ")
                .append(Integer.toString(bladeSurfaceArea));
            sb.append("\n# Blades: ")
                .append(Integer.toString(attachedRotorBlades.size()));
            sb.append("\n# Shafts: ")
                .append(Integer.toString(attachedRotorShafts.size()));
            sb.append("\nRotor Drag CoEff: ")
                .append(Float.toString(rotorDragCoefficient));
            sb.append("\nBlade Drag: ")
                .append(Float.toString(bladeDrag));
            sb.append("\nFrict Drag: ")
                .append(Float.toString(frictionalDrag));
            sb.append("\n\nFluid Tanks:\n");
