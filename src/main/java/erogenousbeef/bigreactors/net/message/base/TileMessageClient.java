...
public class TileMessageClient<TE extends TileEntity> extends WorldMessageClient {
    ...
    protected abstract static class Handler<MESSAGE extends TileMessageClient, TE extends TileEntity>
        extends WorldMessageClient.Handler<MESSAGE> {
        ...
        @Override
        protected IMessage handleMessage(MESSAGE message, MessageContext ctx, TileEntity te) {
            TE concrete = getImpl(te);
            if (concrete != null) {
                return handle(message, ctx, concrete);
            } else {
                BRLog.error(
                    "Received a TileMessageClient for a non-resolvable TileEntity @ %d, %d, %d",
                    message.x,
                    message.y,
                    message.z);
            }
